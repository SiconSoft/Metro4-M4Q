/*
 * Metro 4 Components Library v4.3.0 beta 1 (https://metroui.org.ua)
...
var Metro = {
    ...
    version: "4.3.0",
    versionFull: "4.3.0 beta 1 03/03/2019 19:43:58",
    build: "1",
    isTouchable: isTouch,
...
var d = new Date().getTime();
    ...
    github: function(repo, callback, error){
        var that = this;
        $.get(
            'https://api.github.com/repos/' + repo,
            {},
            function(response, status, xhr){
                that.exec(callback, [JSON.parse(response), status, xhr], response);
            }, function(status, textStatus, xhr){
                that.exec(error, [status, textStatus, xhr], xhr);
            }, "jsonp", {
                'User-Agent': 'request'
            }
        );
...
var Audio = {
    ...
    _createControls: function(){
        ...
        var controls = $("<div>").addClass("controls").insertAfter(element);
...
var CalendarPicker = {
    ...
    _createStructure: function(){
        ...
        if (o.copyInlineStyles === true) {
            $.each(Utils.getInlineStyles(element), function(value, key){
    ...
    _overlay: function(){
        ...
        overlay
            .addClass("overlay")
            .addClass("for-calendar-picker")
            .addClass(o.clsOverlay);
...
var Carousel = {
    ...
    _createSlides: function(){
        ...
        $.each(slides, function(s, i){
            var slide = $(s);
            if (slide.attr("data-cover") !== undefined) {
                slide.css({
    ...
    _effect: function(current, next, effect, to, interval){
        ...
        if (next.attr('data-duration') !== undefined) {
            duration = parseInt(next.attr('data-duration'));
        }

        if (next.attr('data-effect-func') !== undefined) {
            effectFunc = next.attr('data-effect-func');
        }
        ...
        if (effect === 'slide' && !o.revers) {
            func = to === 'next' ? 'slideLeft': 'slideRight';
        }

        if (effect === 'slide' && o.revers) {
            func = to === 'next' ? 'slideRight': 'slideLeft';
        }

        if (effect === 'slide-v' && !o.revers) {
            func = to === 'next' ? 'slideUp': 'slideDown';
        }

        if (effect === 'slide-v' && o.revers) {
            func = to === 'next' ? 'slideDown': 'slideUp';
        }
...
var Collapse = {
    ...
    _close: function(el){
        ...
        el.slideUp(options.duration, "linear", function(){
    ...
    _open: function(el){
        ...
        el.slideDown(options.duration, "linear", function(){
...
var Countdown = {
    ...
    _build: function(){
        ...
        delta_days = Math.ceil((that.breakpoint - now) / dm);

        $.each(parts, function(partName){
            var part = $("<div>").addClass("part " + partName).addClass(o.clsPart).attr("data-label", that.locale["calendar"]["time"][partName]).appendTo(element);

            if (partName === "days") {part.addClass(o.clsDays);}
            if (partName === "hours") {part.addClass(o.clsHours);}
            if (partName === "minutes") {part.addClass(o.clsMinutes);}
            if (partName === "seconds") {part.addClass(o.clsSeconds);}

            $("<div>").addClass("digit").appendTo(part);
            $("<div>").addClass("digit").appendTo(part);

            if (partName === "days" && delta_days >= 100) {
...
var Cube = {
    ...
    _start: function(){
        ...
        $.each(this.rules, function(rule, index){
...
var Keypad = {
    ...
    _createKeypad: function(){
        var element = this.element, o = this.options;
        var keypad = $("<div>").addClass("input").addClass(element[0].className),
            keys;
    ...
    _createKeys: function(){
        var o = this.options,
            keypad = this.keypad,
            factor = Math.round(Math.sqrt(this.keys.length + 2)),
            key_size = o.keySize,
...
var Slider = {
    ...
    _marker: function(){
        ...
        if (o.vertical === true) {
            if (slider_visible) {
                marker.css('top', (length - this.pixel) + 'px') ;
            } else {
    ...
    touchEnd: function(e) {
        ...
        if (touches) {
            if (touches.length && !this.inMultiFingerRelease()) {
                this.startMultiFingerRelease(e);
                return true;
        ...
        //We dont run trigger handler as it will re-trigger events that may have fired already
            ...
            this.phase = TouchConst.PHASE_END;
            this.triggerHandlerForGesture(e, this.phase, TouchConst.TAP);
